...
const InputFieldCssClasses = {
    ...
    INPUT_COLOR: 'input__color',
};

class InputColorFieldCreator {
    ...
    public createElement({
        ...
        if (this.inputElement) this.inputElement.classList.add(InputFieldCssClasses.INPUT_COLOR);
    ...
    public setCallback(callback: (event: Event) => void): void {
        ...
        if (typeof callback === 'function') {
            this.inputElement.addEventListener('input', (event) => callback(event));
        }
    }
...
export default InputColorFieldCreator;
